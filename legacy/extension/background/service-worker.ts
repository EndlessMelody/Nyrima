...
// to deliver a longer app session we silent-refresh via prompt=none every
// ~50min and force a fresh interactive consent after our session ceiling.
...
// Distinct error code the frontend recognises so it can surface "your session
...
async function silentRefreshIfEligible(): Promise<void> {
  ...
  //   - they're within the interactive window,
  ...
  try {
    ...
    if (isOAuthInteractiveSessionExpired(interactiveAt)) return;
...
async function readOAuthLoginHint(): Promise<string | null> {
  try {
    const obj = await chrome.storage.local.get(STORAGE_KEYS.USER_PROFILE);
    return getStoredOAuthLoginHint(obj[STORAGE_KEYS.USER_PROFILE]);
  } catch {
    return null;
  }
}
...
async function getAuthToken(interactive: boolean): Promise<string> {
  ...
  // Interactive ceiling. Once the wall-clock window since the last
  ...
  const interactiveAt = await readInteractiveAt();
  const interactiveExpired = isOAuthInteractiveSessionExpired(interactiveAt);
  if (!interactive && interactiveExpired) {
  ...
  // For non-interactive refreshes, hint the previous Google account so a
  // browser with multiple signed-in accounts can avoid account selection.
  const redirectUri = chrome.identity.getRedirectURL();
  const loginHint = interactive ? null : await readOAuthLoginHint();
      ...
      (redirectUrl) => {
          ...
          reject(
            new Error(
              chrome.runtime.lastError?.message ||
                "OAuth flow failed or cancelled.",
            ),
          );
          return;
        }
        ...
        try {
          ...
          if (token) {
            ...
            // Interactive consent (re)starts the app wall-clock window. Silent
            // refreshes don't touch this — they just rotate the underlying 1h
            // access token until the ceiling fires.
            if (interactive) {
